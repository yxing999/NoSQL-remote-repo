...
void showUsage()
{
  ...
  out << "\n    - v : display function metrics violations";
...
bool CodeAnalysisExecutive::ProcessCommandLine(int argc, char* argv[])
{
  ...
  try {
    ...
    //{
    //  std::cout << "\n\n  path \"" << path_ << "\" does not exist at line " << __LINE__ << ", " << __FUNCTION__ << "\n\n";
    //  return false;
    //}
    for (int i = 2; i < argc; ++i)
    {
      if (argv[i][0] == '/' && std::string(argv[i]).size() > 1)
...
void CodeAnalysisExecutive::showActivity(const File& file)
{
  ...
  if (Rslt::running())
  {
    ...
    std::cout << std::left << "\r     Processing file: " << std::setw(80) << FileSystem::Path::getName(file);
  }
}
...
{
  if (std::find(options_.begin(), options_.end(), 'v') == options_.end())
    return;
  flushLogger();
  std::ostringstream out;
...
void CodeAnalysisExecutive::setDisplayModes()
{
  for (auto opt : options_)
  {
    ...
    case 'f':
      ...
      break;
    default:
      if (opt != 'a' && opt != 'b' && opt != 'd' && opt != 'f' && opt != 'm' && opt != 'r' && opt != 's' && opt != 'v')
      {
        std::cout << "\n\n  unknown option " << opt << "\n\n";
      }
    }
