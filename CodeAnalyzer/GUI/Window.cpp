...
void WPFCppCliDemo::getUserSettings()
{
  ...
  try
  {
    sr = gcnew System::IO::StreamReader(settingsPath);
    ...
    userSettings_.violations = stringToBool(sr->ReadLine());
  ...
  catch (Exception^ /*e*/)
  {
    ...
    userSettings_.upattStrs = "*h, *.cpp";
    userSettings_.cl = false;
    userSettings_.violations = true;
...
void WPFCppCliDemo::saveUserSettings()
{
  ...
  try
  {
    sr = gcnew System::IO::StreamWriter(settingsPath);
    ...
    sr->WriteLine(boolToString((bool)hViolations_->IsChecked));
...
void WPFCppCliDemo::setUpDisplaysPanel()
{
  ...
  hViolationsLabel_->Content = "Show Violations Summary";
  hViolations_->IsChecked = userSettings_.violations;
  StackPanel^ hViolationsStack = gcnew StackPanel();
  hViolationsStack->Orientation = Orientation::Horizontal;
  hViolationsStack->Children->Add(hViolations_);
  hViolationsStack->Children->Add(hViolationsLabel_);
  ...
  hStackPanelRight->Children->Add(hViolationsStack);
...
void WPFCppCliDemo::setCmdLineArgs()
{
  ...
  {
    for (size_t i = 0; i < (size_t)hUPattList_->Items->Count; ++i)
    {
