...
*  --------------------
*  ver 3.3 : 17 Sep 2017
*  - fixed bug in getUserSettings() that occurred if there was no userSettings.txt file
*  ver 3.2 : 27 Oct 2016
...
using UserSettings = ref struct Settings { 
  ...
  bool violations;               // show metrics violations
...
namespace CppCliWindows
{
  ref class WPFCppCliDemo : Window
  {
    ...
    CheckBox^ hViolations_ = gcnew CheckBox();
    ...
    Label^ hViolationsLabel_ = gcnew Label();
